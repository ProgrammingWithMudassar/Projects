            ...
            <section className="w-full bg-brand-primary/20 grid lg:grid-cols-[250px,calc(100%-250px)] py-3" >
                <aside className="w-full min-h-[calc(100vh-95px)] h-full bg-brand-primary rounded-tr-[20px] rounded-br-[20px] px-6 py-4 lg:flex hidden justify-between items-center flex-col" >
                    <div className="w-full flex justify-center items-center flex-col gap-4" >
                        ...
                        {KNavItem?.map((item: any, index: number) =>
                            <NavItem key={index}
                                text={item?.name}
                                active={pathname === item?.route}
                                href={item?.route}
                                icon={<RenderIcons navItem={item?.name} route={item?.route} />}
                            />
                        )}
                    </div>
                </aside>
                <aside className="lg:w-full w-screen px-4" >{children}</aside>
            </section>
            ...
            <Drawer
                ...
                <aside className="w-full h-full bg-brand-secondary flex justify-start items-center flex-col" >
                    ...
                    <div className="w-full flex justify-center items-start flex-col mt-4 px-4" >
                        ...
                        <Button text="Diskonnect" style="bg-black-600 mt-6 w-full rounded-full md:text-[16px] text-[14px]" onClick={() => setConfirmationbox(true)} />
                    </div>
                </aside>
            </Drawer>
            <LogoutConfirmation show={isConfirmOpen} onClose={handleCloseConfirm} title="Do you really wish to LogOut?" route={"/auth/login/?user_type=konnector"} />
        </Fragment>
    )
}
