...
import INavItem from "@/json/InternalNavItems.json"
...
const INavigation = ({ children }: Props) => {
    ...
    return (
        <Fragment>
            <nav className="w-full bg-brand-primary/20 grid" >
                <nav className="px-4 h-[70px] bg-brand-primary rounded-b-[20px] flex justify-between items-center" >
                    ...
                    <aside className="lg:flex hidden justify-center items-center gap-6" >
                        ...
                        <Button text="Diskonnect" style="bg-black-600 rounded-full md:text-[16px] text-[14px]" onClick={handleLogout} />
                    </aside>
            ...
            <section className="w-full bg-brand-primary/20 grid lg:grid-cols-[250px,calc(100%-250px)] py-3" >
                <aside className="w-full min-h-[calc(100vh-95px)] h-full bg-brand-primary rounded-tr-[20px] rounded-br-[20px] px-6 py-4 lg:flex hidden justify-between items-center flex-col" >
                    <div className="w-full flex justify-center items-center flex-col gap-4" >
                        ...
                        {INavItem?.map((item: any, index: number) => <NavItem key={index} text={item?.name} active={pathname === item?.route || pathname.includes(`admin/${item?.name}`)} href={item?.route} icon={<RenderIcons navItem={item?.name} route={item?.route} />} />)}
                    </div>
                </aside>
            ...
            >
                <aside className="w-full h-full bg-brand-secondary flex justify-start items-center flex-col" >
                    ...
                    <div className="w-full flex justify-center items-start flex-col mt-4 px-4" >
                        {[...NavItemMetaData, ...INavItem]?.map((item: any, index: number) => <Link onClick={() => setMenu(!isMenu)} key={index} href={item.route} className="w-full h-[46px] text-[16px] text-white-600 border-b-[1px] border-white-600 tracking-wider flex justify-start items-end pb-1 capitalize" >
                            {item?.name}
                        </Link>)}
            ...
            <LogoutConfirmation show={isConfirmOpen} onClose={handleCloseConfirm}  title="Do you really wish to LogOut?"  route={"/auth/login?user_type=internal"}/>
        </Fragment>
    )
}
