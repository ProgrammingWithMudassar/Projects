...
import ANavItem from "@/json/AdminNavItems.json"
...
const ANavigation = ({ children }: Props) => {
    ...
    return (
        <Fragment>
            ...
            <section className="w-full bg-brand-primary/20 grid lg:grid-cols-[250px,calc(100%-250px)] py-3" >
                <aside className="w-full min-h-[calc(100vh-95px)] h-full bg-brand-primary rounded-tr-[20px] rounded-br-[20px] px-6 py-4 lg:flex hidden justify-between items-center flex-col" >
                    <div className="w-full flex justify-center items-center flex-col gap-4" >
                        ...
                        {ANavItem?.map((item: any, index: number) => <NavItem key={index} text={item?.name} active={pathname === item?.route || pathname.includes(`admin/${item?.name}`)} href={item?.route} icon={<RenderIcons navItem={item?.name} route={item?.route} />} />)}
                    </div>
                </aside>
            ...
            >
                <aside className="w-full h-full bg-brand-secondary flex justify-start items-center flex-col" >
                    ...
                    <div className="w-full flex justify-center items-start flex-col mt-4 px-4" >
                        {[...NavItemMetaData, ...ANavItem]?.map((item: any, index: number) => <Link onClick={() => setMenu(!isMenu)} key={index} href={item.route} className="w-full h-[46px] text-[16px] text-white-600 border-b-[1px] border-white-600 tracking-wider flex justify-start items-end pb-1 capitalize" >
                            {item?.name}
                        </Link>)}
            ...
            <LogoutConfirmation show={isConfirmOpen} onClose={handleCloseConfirm} title="Do you really wish to LogOut?" route={"/auth/login/?user_type=admin"} />
        </Fragment>
    )
}
